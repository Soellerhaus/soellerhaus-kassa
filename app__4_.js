        ...
        if (supabaseClient && isOnline) {
            try {
                ...
                if (error) {
                    ...
                    console.log('âœ… Gefunden für', letter + ':', deduplicated.length, '(nach Deduplizierung)');
...
const Buchungen = {
    ...
    async create(artikel, menge=1) {
        ...
        console.log('âœ…', menge, 'Einzelbuchung(en) erstellt für:', artikel.name);
    ...
    async bucheAufAlle(artikel_id, beschreibung = 'Umlage') {
        ...
        for (const gast of alleGäste) {
            ...
            if (insertError) {
                ...
                console.log('âœ… Umlage gebucht für:', gastName, data);
                erfolg++;
            }
        }
...
const Artikel = {
    ...
    async delete(id) { 
        ...
        if (supabaseClient && isOnline) {
            ...
            console.log('âœ… Artikel ' + id + ' in Supabase gelöscht');
        }
...
const UI = {
    ...
    renderNameList(gäste, onSelect) {
        if (!gäste?.length) return `<div class="name-list-empty"><p>Keine Einträge</p><button class="btn btn-secondary btn-block" onclick="handleBackToLogin()">â† Zurück</button></div>`;
...
Router.register('admin-auffuellliste', async () => {
    console.log('ðŸ“‹ Auffüllliste Route - isAdmin:', State.isAdmin);
    if (!State.isAdmin) { 
    ...
    try {
        ...
        console.log('âœ… Auffüllliste geladen:', liste.length, 'Positionen');
    } catch(e) {
        console.error('âŒ Auffüllliste Fehler:', e);
    ...
    <div class="main-content">
        ...
        <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:24px;">
            ...
            <button class="btn btn-success" onclick="resetAuffuelllisteOhneExport()" style="padding:16px;font-size:1.1rem;background:#27ae60;">
                âœ… Auffüllliste zurücksetzen<br>
        ...
        <div class="card mb-3" style="background:#fff3cd;border:1px solid #ffc107;">
            <div style="padding:12px;">
                <p style="margin:0;color:#856404;font-size:0.85rem;">
                    ...
                    Im Druckdialog â†’ "EPSON TM-T88V" wählen â†’ Haekchen bei "Als Standard speichern"
                </p>
            </div>
        </div>
...
window.printAuffuellliste = async () => {
    try {
        ...
        console.log('âœ… Auffüllliste für Druck geladen:', liste.length, 'Positionen');
...
window.resetAuffuelllisteOhneExport = async () => {
    ...
    try {
        ...
        Utils.showToast('âœ… Auffüllliste zurückgesetzt', 'success');
...
window.handleDeleteBuchungenByDate = async (datum) => {
    ...
    try {
        ...
        await DataProtection.createBackup();
        Utils.showToast(`âœ… ${buchungenIds.length} Buchungen vom ${datum} gelöscht`, 'success');
        Router.navigate('admin-alle-buchungen');
    } catch (e) {
        Utils.showToast('Fehler: ' + e.message, 'error');
    }
...
window.bucheUmlageFürAlle = async () => {
    ...
    if (ausgenommen > 0) {
        Utils.showToast(`âœ… Umlage: ${Utils.formatCurrency(preisProGast)} auf ${erfolg} Gäste verteilt (${ausgenommen} ausgenommen)`, 'success');
    } else {
        Utils.showToast(`âœ… Umlage: ${Utils.formatCurrency(preisProGast)} auf ${erfolg} Gäste verteilt`, 'success');
    }
...
window.addGruppe = async (name) => {
    try {
        ...
        console.log('âœ… Gruppe hinzugefügt:', result);
        Utils.showToast(`Gruppe "${name}" hinzugefuegt`, 'success');
        Router.navigate('admin-gruppen');
    } catch (e) {
        console.error('âŒ Gruppe hinzufügen Fehler:', e);
        Utils.showToast(e.message, 'error');
    }
};
...
Router.register('admin-tagesmenu', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†</button><div class="header-title"> Tagesmenü</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
Router.register('admin-cheese', async () => {
    ...
    <div class="app-header">
        <div class="header-left">
            <button class="btn btn-secondary" onclick="Router.navigate('admin-dashboard')">â† Zurück</button>
        </div>
        ...
        try {
            ...
            if (error) {
                ...
                const uniqueData = data.filter(g => {
                    ...
                    if (seenNames.has(name)) {
                        console.log('âš ï¸ Duplikat übersprungen:', name);
                        return false;
                    }
                    seenNames.add(name);
                    return true;
                ...
                console.log('âœ… Gäste von Supabase geladen:', guests.length, '(nach Deduplizierung)');
            }
        } catch(e) {
    ...
    UI.render(`
        <div class="admin-container" style="max-width:900px;margin:0 auto;padding:16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                ...
                <button class="btn btn-secondary" onclick="Router.navigate('admin')">â† Zurück</button>
            </div>
...
window.syncFromCloud = async (silent = false) => {
    ...
    try {
        ...
        console.log('âœ… Lokaler Gäste-Cache gelöscht');
...
window.clearLocalCache = async () => {
    ...
    try {
        ...
        localStorage.clear();
        Utils.showToast('âœ… Cache gelöscht - Seite wird neu geladen', 'success');
...
window.setZeitbegrenzung = async (artikelId, stunden) => {
    try {
        ...
        if (stunden > 0) {
            ...
            Utils.showToast(`âœ… Artikel aktiv für ${zeitText}`, 'success');
        } else {
...
window.changeArtikelId = async (alteId) => {
    ...
    try {
        ...
        Utils.showToast(`âœ… Artikel-ID geändert: ${alteId} â†’ ${neueId}`, 'success');
        Router.navigate('admin-articles');
    } catch (e) {
...
window.migrateLocalToSupabase = async () => {
    ...
    try {
        ...
        console.log(`ðŸ“Š Migration: ${hochgeladen} hochgeladen, ${uebersprungen} übersprungen, ${fehler} Fehler`);
