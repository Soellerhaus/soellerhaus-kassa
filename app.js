...
const i18n = {
    ...
    renderLangButton(showSupport = false) {
        const flag = this.currentLang === 'de' ? '🇬🇧' : '🇩🇪';
        const label = this.currentLang === 'de' ? 'EN' : 'DE';
        
        const supportBtn = showSupport ? `
        ...
        return `
        <div style="position:fixed;top:10px;right:10px;display:flex;align-items:center;gap:8px;z-index:1000;">
            ${supportBtn}
...
const SyncManager = {
    ...
    getAmpelHtml(kompakt = false) {
        return `<div id="sync-ampel" data-kompakt="${kompakt}" onclick="SyncManager.showDetails()" style="cursor:pointer;display:flex;align-items:center;gap:${kompakt ? '0' : '6'}px;padding:4px ${kompakt ? '6' : '10'}px;border-radius:20px;background:rgba(255,255,255,0.9);font-size:0.8rem;"></div>`;
    },
    ...
    async updateUI() {
        ...
        const kompakt = ampelEl.dataset.kompakt === 'true';
        ...
        // Im kompakten Modus nur Icon, sonst Icon + Text
        if (kompakt) {
