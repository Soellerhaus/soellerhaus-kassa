...
const CheeseOrders = {
    ...
    async isEnabled() {
        try {
            if (supabaseClient && isOnline) {
                const { data } = await supabaseClient.from('settings').select('value').eq('key', 'kaese_aktiv').single();
                if (data?.value !== undefined) return data.value === true || data.value === 'true';
            }
            const local = await db.settings.get('kaese_aktiv');
    ...
    async setEnabled(aktiv) {
        await db.settings.put({ key: 'kaese_aktiv', value: aktiv });
        if (supabaseClient && isOnline) {
            try {
                await supabaseClient.from('settings').upsert({ key: 'kaese_aktiv', value: aktiv });
            } catch(e) { console.warn('kaese_aktiv sync:', e); }
        }
    },
...
const SmartHome = {
    ...
    renderInlineHtml(geraete) {
        if (!geraete || geraete.length === 0) return '';
        return `
        <div style="max-width:600px;margin:0 auto 24px;background:linear-gradient(135deg,#3498db,#2c3e50);border-radius:16px;padding:14px 14px 8px;color:white;box-shadow:0 4px 16px rgba(52,152,219,0.25);">
            <div style="font-weight:800;font-size:1.05rem;margin-bottom:10px;display:flex;align-items:center;gap:8px;">
                <span style="font-size:1.4rem;">🏠</span>
                <span>Smart Home</span>
            </div>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;">
                ${geraete.map(g => `
                <div style="background:rgba(255,255,255,0.95);border-radius:12px;padding:12px;color:#222;">
                    <div style="font-weight:700;font-size:0.95rem;margin-bottom:8px;display:flex;align-items:center;gap:6px;">
                        <span style="font-size:1.3rem;">${g.icon || '⚡'}</span>
                        <span>${g.name}</span>
                    </div>
                    <div style="display:flex;gap:6px;">
                        <button onclick="SmartHome.action('${g.id}','on',this)" style="flex:1;background:linear-gradient(135deg,#27ae60,#229954);color:white;border:none;border-radius:8px;padding:10px;font-size:0.95rem;font-weight:700;cursor:pointer;">▶ EIN</button>
                        <button onclick="SmartHome.action('${g.id}','off',this)" style="flex:1;background:linear-gradient(135deg,#7f8c8d,#566060);color:white;border:none;border-radius:8px;padding:10px;font-size:0.95rem;font-weight:700;cursor:pointer;">■ AUS</button>
                    </div>
                </div>
                `).join('')}
            </div>
...
Router.register('admin-dashboard', async () => {
    ...
    <div class="main-content">
        ...
        <!-- TOP-ROW: Gäste, Letzte Buchungen, Auffüllliste, Nachricht -->
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
            <div style="position:relative;">
                <button class="btn btn-block" onclick="Router.navigate('admin-cheese')" style="width:100%;padding:12px;font-size:0.95rem;font-weight:600;background:linear-gradient(135deg, #f4d03f, #f39c12);color:#5d4e37;border:none;border-radius:10px;box-shadow:0 2px 8px rgba(243,156,18,0.3);${!kaeseAktiv ? 'opacity:0.5;' : ''}${cheeseCount > 0 ? 'animation:cheesePulse 1.5s ease-in-out infinite;' : ''}">
                    🧀 Käse ${cheeseCount > 0 ? '<span style="background:#e74c3c;color:white;padding:2px 8px;border-radius:12px;font-size:0.75rem;margin-left:4px;">' + cheeseCount + '</span>' : ''}
                </button>
                <button onclick="toggleKaese()" style="position:absolute;top:4px;right:4px;background:${kaeseAktiv ? '#27ae60' : '#e74c3c'};color:white;border:none;border-radius:12px;padding:2px 8px;font-size:0.65rem;font-weight:700;cursor:pointer;">${kaeseAktiv ? 'AN' : 'AUS'}</button>
            </div>
...
Router.register('buchen', async () => {
    ...
    ">
        ...
        <div onclick="toggleWarenkorbDetails()" style="padding:18px 20px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;">
    ...
    UI.append(`<div style="padding:4px 12px 160px 12px;max-width:1200px;margin:0 auto;">
        ...
        ${fehlendeOffen.length ? (() => {
            ...
            const artIconMap = {};
            sichtbareArts.concat(arts).forEach(a => { artIconMap[a.name] = a; });
            ...
            const gesamtFehlend = fehlendeOffen.reduce((s,f) => s + (f.artikel_preis || 0), 0);
            
            return `
        <div style="background:linear-gradient(135deg, #e67e22, #d35400);border-radius:14px;margin-bottom:16px;overflow:hidden;">
            <div onclick="document.getElementById('fehlende-accordion').style.display = document.getElementById('fehlende-accordion').style.display === 'none' ? 'block' : 'none'; document.getElementById('fehlende-arrow').textContent = document.getElementById('fehlende-accordion').style.display === 'none' ? '▼' : '▲';" style="padding:12px 16px;color:white;cursor:pointer;display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:10px;">
                ...
                <span id="fehlende-arrow" style="font-size:0.9rem;">▼</span>
            </div>
            <div id="fehlende-accordion" style="display:none;max-height:300px;overflow-y:auto;">
                ${sortDaten.map(datum => {
                    ...
                    return `
                    <div style="background:rgba(255,255,255,0.95);padding:8px 14px;border-top:1px solid rgba(0,0,0,0.08);">
                        <div style="font-weight:700;color:#d35400;font-size:0.8rem;margin-bottom:6px;">📅 ${tagName} ${datum}</div>
                        <div style="display:flex;flex-wrap:wrap;gap:5px;">
                            ${Object.values(grouped).map(g => `
                            <button onclick="uebernehmeFehlend(${g.ids[0]})" style="background:#fff3e0;color:#333;border:1px solid #e67e22;border-radius:8px;padding:5px 10px;cursor:pointer;display:flex;align-items:center;gap:4px;font-size:0.8rem;" onmousedown="this.style.transform='scale(0.95)'" onmouseup="this.style.transform='scale(1)'" title="Klick = 1 Stück übernehmen${g.count > 1 ? ' (noch ' + g.count + ' übrig)' : ''}">
        ...
        <div class="category-tabs" style="margin-bottom:8px;padding:6px 0;">
