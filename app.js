...
// ===========================================
// TAGESMENÃœ - MenÃ¼ auf der Startseite anzeigen
// ===========================================
const TagesMenu = {
    // PrÃ¼fen ob eine Uhrzeit aktuell Ã¼berschritten ist
    ...
    async speichern(menuData) {
        ...
        Utils.showToast('ðŸ½ MenÃ¼ gespeichert!', 'success');
        return menuData;
    },
    ...
    renderButtons(menus) {
        ...
        if (menus.mittag) {
            html += `<button onclick="TagesMenu.showModal('mittag')" style="
                ...
                margin: 0 auto 12px;
            ">
                <span style="font-size:1.4rem;">ðŸŒž</span>
    ...
    async showModal(typ = 'mittag') {
        ...
        const icon = typ === 'mittag' ? 'ðŸŒž' : 'ðŸŒ™';
...
const CheeseOrders = {
                ...
                <div style="padding:20px;">
                    ...
                    <div style="margin-bottom:20px;">
                        ...
                        <div style="text-align:center;margin-top:8px;font-size:0.85rem;color:#888;">
                            Min: 100g â€¢ Max: 1kg â€¢ Schritte: 50g<br>
                            <span style="color:#e67e22;">âš–ï¸ Grammzahl ist ca.-Angabe (Richtwert)</span>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom:20px;">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                            ...
                            ">
                                ...
                                <div style="color:#e67e22;font-weight:700;">+1,00 â‚¬</div>
                            </label>
                        </div>
                    </div>
                    ...
                    ">
                        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                            <span>KÃ¤se (ca. <span id="cheese-price-grams">100</span>g Ã— 2,50 â‚¬/100g)</span>
                            <span id="cheese-price-base">2,50 â‚¬</span>
                        </div>
                        ...
                        <div style="border-top:1px solid rgba(255,255,255,0.3);padding-top:12px;margin-top:8px;">
                            <div style="display:flex;justify-content:space-between;font-size:1.3rem;font-weight:700;">
                                ...
                                <span id="cheese-price-total">2,50 â‚¬</span>
                            </div>
                        </div>
                    </div>
    ...
    async submitOrder() {
        ...
        try {
            ...
            // ErfolgsbestÃ¤tigung
            Utils.showToast('ðŸ§€ KÃ¤se-Bestellung erfolgreich!', 'success');
...
const Utils = {
    ...
    getBuchungsDatum() {
        ...
        // Vor 7 Uhr morgens? â†â€™ Datum vom Vortag verwenden
        if (stunde < 7) {
...
const RegisteredGuests = {
    ...
    async login(id, password) {
        ...
        if (supabaseClient && isOnline) {
            ...
            if (!profile.email) {
                // Gast wurde ohne Auth User angelegt - nur PIN prÃ¼fen (Legacy-Support)
                ...
                const user = { 
                    ...
                    noAuthSession: true // Flag fÃ¼r Buchungslogik
                };
                State.setUser(user);
...
const Buchungen = {
    ...
    async create(artikel, menge=1) {
        ...
        console.log('ðŸ“ Buchung erstellen fÃ¼r User:', userId, 'Menge:', menge);
...
const SyncManager = {
    ...
    async syncPending() {
            ...
            for (const b of pending) {
                try {
                    ...
                    if (error) {
                        ...
                        await db.buchungen.update(b.buchung_id, { sync_status: 'synced' });
                        synced++;
                        console.log('âœ… Synced:', b.buchung_id);
                    }
                } catch(e) {
        ...
        if (synced > 0) {
            console.log(`âœ… SyncManager: ${synced} synchronisiert, ${failed} fehlgeschlagen`);
            this.updateUI();
        }
    ...
    async showDetails() {
        ...
        let message = '';
        if (status === 'green') {
            message = 'âœ… Alle deine Buchungen sind erfolgreich synchronisiert!';
        } else if (status === 'yellow') {
            message = `ðŸŸ¡ ${pending} Buchung(en) warten noch auf Upload.\n\nDie App versucht automatisch, diese hochzuladen.`;
        } else {
...
const Artikel = {
    ...
    async getAll(f={}) {
        ...
        // Von Supabase laden wenn online UND (Cache null ODER Ã¤lter als 30 Sekunden)
...
Router.register('register', () => {
    ...
    UI.render(`<div class="main-content"><div style="max-width:500px;margin:40px auto;">
        ...
        <button class="btn btn-secondary btn-block mt-3" onclick="handleBackToLogin()">â†Â ${t('back')}</button>
    </div></div>`);
});
...
Router.register('pin-entry', () => {
    ...
    UI.render(`<div class="main-content"><div style="max-width:500px;margin:60px auto;">
        ...
        <button class="btn btn-secondary btn-block mt-3" onclick="handlePinCancel()">â†Â ${t('back')}</button>
    </div></div>`);
});
...
Router.register('gruppe-waehlen', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><div class="header-title">ðŸ« ${t('select_group')}</div></div><div class="header-right"><button class="btn btn-secondary" onclick="Auth.logout()">${t('cancel')}</button></div></div>
...
Router.register('admin-dashboard', async () => {
    ...
    <div class="main-content">
        ...
        " onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 25px rgba(0,0,0,0.3)'" onmouseout="this.style.transform='';this.style.boxShadow='0 4px 15px rgba(0,0,0,0.2)'">
            <div style="display: flex; align-items: center; justify-content: space-between;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <span style="font-size: 2rem;">${isHP ? 'ðŸ½' : 'ðŸ '}</span>
                    <div>
        ...
        " onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 8px 25px rgba(0,0,0,0.3)'" onmouseout="this.style.transform='';this.style.boxShadow='0 4px 15px rgba(0,0,0,0.2)'">
            <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <span style="font-size: 1.5rem;">â†â€™</span>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px;">
            ...
            <button class="btn btn-primary" onclick="Router.navigate('admin-artikel-sortierung')" style="padding:24px;">ðŸ”€ Artikel sortieren</button>
...
Router.register('admin-auffuellliste', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸº AuffÃ¼llliste</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
        <div class="card mb-3" style="background:var(--color-alpine-green);color:white;">
            <div style="padding:20px;text-align:center;">
        ...
        <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:24px;">
            <button class="btn btn-primary" onclick="printAuffuellliste()" style="padding:16px;font-size:1.1rem;">
                ðŸ–¨ï¸ FÃ¼r Thermodrucker drucken
            </button>
        ...
        <div class="card mb-3" style="background:#fff3cd;border:1px solid #ffc107;">
            <div style="padding:12px;">
                <p style="margin:0;color:#856404;font-size:0.85rem;">
                    ðŸ–¨ï¸ <strong>Tipp:</strong> Damit der Thermodrucker automatisch gewÃ¤hlt wird:<br>
                    Im Druckdialog â†â€™ "EPSON TM-T88V" wÃ¤hlen â†â€™ HÃ¤kchen bei "Als Standard speichern"
                </p>
            </div>
        </div>
...
window.printAuffuellliste = async () => {
    ...
    printWindow.document.write(`
        ...
        <body>
            ...
            <div class="total">GESAMT: ${total} STÃœCK</div>
...
Router.register('admin-alle-buchungen', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ“‹ Alle Buchungen</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
        <div class="card mb-3" style="background:var(--color-alpine-green);color:white;">
...
Router.register('admin-fehlende', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">âš   Fehlende GetrÃ¤nke</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
Router.register('admin-umlage', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ’° Umlage buchen</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
Router.register('admin-notfall-export', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ”§ Notfall-Export</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
Router.register('admin-gruppen', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ« Gruppenverwaltung</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
Router.register('admin-preismodus', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ’° Preismodus</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
        ...
        <div class="card mb-3" style="background:${isHP ? 'linear-gradient(135deg, #9b59b6, #8e44ad)' : 'linear-gradient(135deg, #3498db, #2980b9)'};color:white;">
            <div style="padding:24px;text-align:center;">
                <div style="font-size:4rem;margin-bottom:16px;">${isHP ? 'ðŸ½' : 'ðŸ '}</div>
...
Router.register('admin-nachricht', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ“¢ GÃ¤ste-Nachricht</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
Router.register('admin-tagesmenu', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ½ TagesmenÃ¼</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
...
KÃ¤sespÃ¤tzle mit RÃ¶stzwiebeln
...
Dessert" style="font-size:0.95rem;line-height:1.5;">${menuData.mittag?.text || ''}</textarea>
                ...
                <div style="display:flex;gap:12px;">
                    <button class="btn ${menuData.mittag?.aktiv ? 'btn-secondary' : 'btn-primary'}" onclick="toggleMittagMenu()" style="flex:1;padding:14px;">
                        ${menuData.mittag?.aktiv ? 'âŒ Deaktivieren' : 'âœ… Aktivieren'}
                    </button>
                    ${menuData.mittag?.aktiv ? `<button class="btn btn-primary" onclick="TagesMenu.showModal('mittag')" style="padding:14px;">ðŸ‘ï¸ Vorschau</button>` : ''}
                </div>
            </div>
        </div>
...
Apfelstrudel mit Vanillesauce" style="font-size:0.95rem;line-height:1.5;">${menuData.abend?.text || ''}</textarea>
                ...
                <div style="display:flex;gap:12px;">
                    <button class="btn ${menuData.abend?.aktiv ? 'btn-secondary' : 'btn-primary'}" onclick="toggleAbendMenu()" style="flex:1;padding:14px;">
                        ${menuData.abend?.aktiv ? 'âŒ Deaktivieren' : 'âœ… Aktivieren'}
                    </button>
                    ${menuData.abend?.aktiv ? `<button class="btn btn-primary" onclick="TagesMenu.showModal('abend')" style="padding:14px;">ðŸ‘ï¸ Vorschau</button>` : ''}
                </div>
            </div>
        </div>
...
Router.register('admin-cheese', async () => {
    ...
    const orderRows = orders.length ? orders.map(o => {
        ...
        const verpackung = o.packaging_type === 'VACUUM' ? 'ðŸ“¦ Vakuum' : 'ðŸŽ Folie';
    ...
    <div class="app-header">
        ...
        <div class="header-title">ðŸ§€ KÃ¤se-Bestellungen</div>
        <div class="header-right">
            <button class="btn btn-secondary" onclick="Router.navigate('admin-cheese')">ðŸ”„</button>
        </div>
    </div>
    <div class="main-content">
        <!-- Statistik -->
        ...
        <div style="margin-top:20px;padding:16px;background:#fff3cd;border-radius:10px;font-size:0.9rem;color:#856404;">
            <strong>ðŸ’¡ Hinweis:</strong> Erledigte Bestellungen werden aus dieser Liste entfernt, bleiben aber fÃ¼r Auswertungen in der Datenbank erhalten.
        </div>
    </div>`);
});
...
Router.register('admin-guests', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ‘¥ GÃ¤steverwaltung</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
        <style>
...
Router.register('admin-guests-inaktiv', async () => {
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-guests')">â†â€™</button><div class="header-title">ðŸ“‹ Inaktive GÃ¤ste</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <div class="main-content">
        <div class="card mb-3" style="background:#95a5a6;color:white;">
            <div style="padding:16px;text-align:center;">
...
Router.register('admin-articles', async () => {
    ...
    const renderArticleRow = (a, pos) => {
        ...
        if (a.aktiv_bis) {
            ...
            if (diffMs > 0) {
                if (diffTage > 0) {
                    zeitInfo = `<div style="font-size:0.7rem;color:#e67e22;margin-top:2px;">â° ${diffTage}T Ã¼brig</div>`;
                } else if (diffStunden > 0) {
                    zeitInfo = `<div style="font-size:0.7rem;color:#e74c3c;margin-top:2px;">â° ${diffStunden}h Ã¼brig</div>`;
                } else {
                    zeitInfo = `<div style="font-size:0.7rem;color:#e74c3c;margin-top:2px;">â° <1h Ã¼brig</div>`;
                }
            }
        }
    ...
    UI.render(`<div class="app-header"><div class="header-left"><button class="menu-btn" onclick="Router.navigate('admin-dashboard')">â†â€™</button><div class="header-title">ðŸ“¦ Artikelverwaltung</div></div><div class="header-right"><button class="btn btn-secondary" onclick="handleLogout()">Abmelden</button></div></div>
    <style>
        .switch { position:relative; display:inline-block; width:50px; height:26px; }
        .switch input { opacity:0; width:0; height:0; }
                            ...
                            <tr style="background:var(--color-stone-light);text-align:left;">
                                ...
                                <th style="padding:12px 8px;text-align:center;">
                                    ...
                                    <div style="display:flex;gap:8px;justify-content:center;font-size:0.7rem;font-weight:normal;">
                                        ...
                                        <span style="color:#9b59b6;">ðŸ½ HP</span>
                                    </div>
                                </th>
        ...
        let artikelHtml = artikelInKat.map((a, idx) => `
            <div class="sort-artikel-tile" draggable="true" 
                ...
                <div class="sort-artikel-content">
                    ${a.bild ? `<img src="${a.bild}" class="sort-artikel-img">` : `<div class="sort-artikel-icon">${a.icon || 'ðŸ“¦'}</div>`}
                    <div class="sort-artikel-name">${a.name_kurz || a.name}</div>
                </div>
                <div class="sort-artikel-handle">â‹®â‹®</div>
            </div>
        `).join('');
    ...
    UI.render(`
        <div class="admin-container" style="max-width:900px;margin:0 auto;padding:16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <div>
                    <h1 style="font-size:1.5rem;margin:0;">ðŸ”€ Artikel sortieren</h1>
...
Router.register('buchen', async () => {
    ...
    const warenkorbBtnHtml = sessionBuchungen.length ? `
        ...
        ">
            ðŸ›’ <span style="background:white;color:var(--color-alpine-green);border-radius:50%;width:22px;height:22px;display:flex;align-items:center;justify-content:center;font-size:0.85rem;">${sessionCount}</span>
    ...
    ">
        <div style="padding:10px 14px;background:var(--color-alpine-green);color:white;display:flex;justify-content:space-between;align-items:center;">
            <strong style="font-size:0.95rem;">ðŸ›’ Aktuell gebucht</strong>
        ...
        <div class="buchungen-uebersicht" style="background:var(--color-alpine-green);border-radius:16px;margin-bottom:20px;overflow:hidden;">
            <div onclick="toggleBuchungsDetails()" style="padding:14px;cursor:pointer;display:flex;justify-content:space-between;align-items:center;">
                <div style="color:white;">
                    <div style="font-weight:700;font-size:1rem;">ðŸ“‹ ${t('my_bookings')}</div>
                ...
                <div style="margin-bottom:12px;">
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:6px 8px;background:#f5f5f5;border-radius:6px;margin-bottom:4px;font-size:0.85rem;">
                        <div>
                            ...
                            <div style="font-size:0.75rem;color:#888;">ðŸ• ${b.uhrzeit?.substring(0,5) || ''} â€¢ ${b.menge}Ã—</div>
                        </div>
...
// Dropdown schlieÃŸen wenn auÃŸerhalb geklickt
...
window.artikelPressEnd = (event) => {
    ...
    // Touch-Dauer prÃ¼fen (mindestens 80ms um versehentliches Antippen zu vermeiden)
...
window.changeArtikelId = async (alteId) => {
    ...
    try {
        ...
        Utils.showToast(`âœ… Artikel-ID geÃ¤ndert: ${alteId} â†â€™ ${neueId}`, 'success');
        Router.navigate('admin-articles');
    } catch (e) {
...
window.showAddArticleModal = () => {
    ...
    <div style="background:#f8f9fa;border-radius:12px;padding:16px;margin-bottom:16px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            ...
            <div class="form-group" style="margin-bottom:0;">
                <label class="form-label" style="color:#9b59b6;font-weight:600;">ðŸ½ Halbpension (â‚¬)</label>
...
window.saveEditArticle = async () => {
    ...
    await Artikel.update(id, { 
        ...
        preis: preisSV,
        preis_hp: preisHP,
        steuer_prozent: parseInt(document.getElementById('article-mwst')?.value) || 10,
        kategorie_id: katId, 
