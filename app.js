...
const SyncManager = {
    ...
    // Ampel-HTML generieren (compact = nur Icon, kein Text)
    getAmpelHtml(compact = false) {
        const compactAttr = compact ? 'data-compact="true"' : '';
        return `<div id="sync-ampel" ${compactAttr} onclick="SyncManager.showDetails()" style="cursor:pointer;display:flex;align-items:center;gap:6px;padding:4px ${compact ? '8px' : '10px'};border-radius:20px;background:rgba(255,255,255,0.9);font-size:0.8rem;"></div>`;
    },
    ...
    async updateUI() {
        ...
        const isCompact = ampelEl.hasAttribute('data-compact');
        ...
        // Im compact-Modus nur Icon zeigen
        if (isCompact) {
...
Router.register('register', () => {
    ...
    UI.render(`<div class="app-header">
        <div class="header-left">
            <button class="menu-btn" onclick="handleBackToLogin()">←</button>
            <div class="header-title">${t('register')}</div>
        </div>
        <div class="header-right">
            <button class="btn btn-primary" onclick="handleRegisterSubmit()" style="padding:8px 16px;font-size:0.95rem;">✔ ${t('register_btn')}</button>
        </div>
    </div>
    <div class="main-content" style="padding-top:10px;"><div style="max-width:500px;margin:0 auto;">
        <div class="card" style="padding:16px;">
            ...
            <div class="form-group" style="margin-bottom:0;">
                ...
                <div class="pin-display" id="register-pin-display" style="display:flex;justify-content:center;gap:10px;margin:12px 0;">
                    <div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div>
                </div>
                <div class="pin-buttons" style="gap:6px;">
                    ${[1,2,3,4,5,6,7,8,9].map(n => `<button type="button" class="pin-btn" onclick="handleRegisterPinInput('${n}')" style="width:60px;height:50px;font-size:1.3rem;">${n}</button>`).join('')}
                    <button type="button" class="pin-btn" style="visibility:hidden;width:60px;height:50px;"></button>
                    <button type="button" class="pin-btn" onclick="handleRegisterPinInput('0')" style="width:60px;height:50px;font-size:1.3rem;">0</button>
...
window.openWhatsAppSupport = () => {
    ...
    // Kurze Nachricht
    const nachricht = `Name: ${userName || '___'}
Datum: ${datum}
